...
if (isset($_POST['saveData']) AND $can_read AND $can_write) {
  ...
  if (empty($title)) {
    ...
    if (!empty($_FILES['image']) AND $_FILES['image']['size']) {
      ...
      // File name DISINI
      $nextID = $dbs->query("SHOW TABLE STATUS LIKE 'biblio'");
      $nextID = $nextID->fetch_array()['Auto_increment'];
      $item_idnya = $_GET['itemID'] ? $_GET['itemID'] : $nextID;

      // upload the file and change all space characters to underscore
      $img_upload_status = $image_upload->doUpload('image', "B" . $item_idnya . "_" . preg_replace('@\s+@i', '_', $_FILES['image']['name']));
...
if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail')) {
  ...
  $str_input .= '<iframe name="authorIframe" id="authorIframe" class="borderAll" style="width: 100%; height: 100%;" src="'.MWB.'bibliography/iframe_author.php?biblioID='.$rec_d['biblio_id'].'&block=1"></iframe>';
  ...
  if (!$in_pop_up AND $form->edit_mode) {
    ...
    $str_input .= '<iframe name="itemIframe" id="itemIframe" class="borderAll" style="width: 100%; height: 100%;" src="'.MWB.'bibliography/iframe_item_list.php?biblioID='.$rec_d['biblio_id'].'&block=1"></iframe>'."\n";
  ...
  //if ($rec_d['publisher_id']) {
    $publ_q = $dbs->query(sprintf('SELECT publisher_id, publisher_name FROM mst_publisher', $rec_d['publisher_id']));
  ...
  //if ($rec_d['publish_place_id']) {
    $plc_q = $dbs->query(sprintf('SELECT place_id, place_name FROM mst_place', $rec_d['publish_place_id']));
  ...
  $str_input .= '<iframe name="topicIframe" id="topicIframe" class="borderAll" style="width: 100%; height: 100%;" src="'.MWB.'bibliography/iframe_topic.php?biblioID='.$rec_d['biblio_id'].'&block=1"></iframe>';
  ...
  $str_input .= '<iframe name="attachIframe" id="attachIframe" class="borderAll" style="width: 100%; height: 100%;" src="'.MWB.'bibliography/iframe_attach.php?biblioID='.$rec_d['biblio_id'].'&block=1"></iframe>';
  ...
  if ($can_read AND $can_write) {
    // DISINI print_r($_SESSION);
    if($_SESSION['uid']!=1){
      $datagrid->setSQLCriteria("WHERE index.uid = ".$_SESSION['uid']);
    }
  ...
  if ($can_read AND $can_write) {
    if($_SESSION['uid']!=1){
      $datagrid->setSQLCriteria("WHERE biblio.uid = ".$_SESSION['uid']);
    }
  ...
  $datagrid->setSQLcriteria('('.$criteria['sql_criteria'].')');
      if($_SESSION['uid']!=1){
      $datagrid->setSQLCriteria("WHERE biblio.uid = ".$_SESSION['uid']." AND ".'('.$criteria['sql_criteria'].')');
    }
  }
