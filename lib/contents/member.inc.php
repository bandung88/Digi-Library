    ...
    if (!$username OR !$password) {
        ...
        if ($logon->valid($dbs)) {
            ...
            if($_SESSION['m_is_expired']){
                session_unset();
                header("location:index.php?p=member&memberExpired=true");
                die();   
            }else{
                header('Location: index.php?p=member');
            }
            ...
            if ($_pass_d[0] === $md5_password || $enc_password === $_pass_d[0]) {
                $new_pass = password_hash($password, PASSWORD_BCRYPT);
                $dbs->query('UPDATE member SET mpasswd = \''.$new_pass.'\' WHERE member_id = \''.$username.'\'');
                //echo $dbs->affected_rows;
                if($dbs->error) {
                    $msg  = '';
...
?>
    ...
    // Member is EXPIRED :v
    if (isset($_GET['memberExpired']) && $_GET['memberExpired'] === 'true') {
        echo '<div class="errorBox">'.__('You can\'t Login, Your member status is Expired! Please do confirmation with Librarian!').'</div>';
    }
	?>
