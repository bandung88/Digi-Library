...
class detail
{
    ...
    public function __construct($dbs, $int_detail_id, $str_output_format = 'html')
    {
        ...
        // Update viewer
        $current_viewer = $this->record_detail['viewers'] ? $this->record_detail['viewers'] : 0;
        $this->db->query("UPDATE biblio set viewers = ".($current_viewer+1)." where biblio_id='".$this->detail_id."'");
    ...
    public function getAttachments() {
        ...
        foreach ($this->record_detail['attachments'] as $attachment_d) {
          ...
          if ($attachment_d['mime_type'] == 'application/pdf') {
            ...
            if(strpos(trim($attachment_d['file_url']), "youtube.com") !== FALSE){
              $url = explode("=", trim($attachment_d['file_url']));
              $_output .= '<li class="attachment-audio-video" itemprop="video" property="video" itemscope itemtype="http://schema.org/VideoObject" style="list-style-image: url(images/labels/auvi.png)">'
              .'<a itemprop="name" property="name" class="openPopUp" title="'.$attachment_d['file_title'].'" href="https://www.youtube.com/embed/'.$url[1].'?autoplay=1&color=white" width="640" height="480">'.$attachment_d['file_title'].'</a>';
            }else{
    ...
    protected function htmlOutput()
    {
        ...
        $_share_btns = "\n".'<ul class="share-buttons">'.
            ...
            '<li><i class="fa fa-eye"></i> : '.$this->record_detail['viewers'].'</li>'.
            '</ul>'."\n";
